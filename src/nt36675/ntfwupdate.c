...
int nt36675_set_page(IN SPB_CONTEXT* SpbContext, IN unsigned int pageaddr)
{
...
int nt36675_write_addr(IN SPB_CONTEXT* SpbContext, unsigned int addr, unsigned char data)
{
    int ret = 0;
    ...
    //---set xdata index---
    ret = nt36675_set_page(SpbContext, addr);
    if (ret) {
        return ret;
    }
...
int nt36675_eng_reset(IN SPB_CONTEXT* SpbContext)
{
    LARGE_INTEGER Interval;
    int ret;

    ret = nt36675_write_addr(SpbContext, ENG_RST_ADDR, 0x5A);
    if (ret)
        return ret;
...
int nt36675_bootloader_reset(IN SPB_CONTEXT* SpbContext)
{
    LARGE_INTEGER Interval;
    int ret;

    ret = nt36675_write_addr(SpbContext, SWRST_N8_ADDR, nt36675_CMD_BOOTLOADER_RESET);
    if (ret)
        return ret;
    ...
    ret = nt36675_write_addr(SpbContext, SPI_RD_FAST_ADDR, 0x00);
    if (ret)
        return ret;
    ...
    unsigned int G_CHECKSUM_ADDR, unsigned int crc, SPB_CONTEXT* SpbContext)
{
    ...
    nt36675_set_page(SpbContext, DES_ADDR);
...
void nvt_bld_crc_enable(SPB_CONTEXT* SpbContext)
{
    ...
    nt36675_set_page(SpbContext, BLD_CRC_EN_ADDR);
...
void nvt_fw_crc_enable(SPB_CONTEXT* SpbContext)
{
    ...
    nt36675_set_page(SpbContext, EVENT_BUF_ADDR);

    //---clear fw reset status---
    buf[0] = 0x00;
    SpbWriteDataSynchronously(SpbContext, SPI_WRITE_MASK(nt36675_EVT_RESET_COMPLETE & 0x7F), buf, 1);

    //---enable fw crc---
    buf[0] = 0xAE;	//enable fw crc command
    SpbWriteDataSynchronously(SpbContext, SPI_WRITE_MASK(nt36675_EVT_HOST_CMD & 0x7F), buf, 1);
}
...
void nvt_boot_ready(SPB_CONTEXT* SpbContext)
{
    ...
    nt36675_write_addr(SpbContext, BOOT_RDY_ADDR, 1);
...
void disable_pen_input_device(SPB_CONTEXT* SpbContext, BOOLEAN disable)
{
    ...
    //---set xdata index to EVENT BUF ADDR---
    nt36675_set_page(SpbContext, EVENT_BUF_ADDR | nt36675_EVT_HOST_CMD);

    //---clear fw reset status---
    buf[0] = 0x7B;
    buf[1] = disable;
    SpbWriteDataSynchronously(SpbContext, SPI_WRITE_MASK(nt36675_EVT_HOST_CMD & 0x7F), buf, 2);
}
...
NVTLoadFirmwareFile(WDFDEVICE FxDevice, SPB_CONTEXT* SpbContext) {
    ...
    nt36675_bootloader_reset(SpbContext);
    ...
    //nt36675_write_addr(SpbContext, TX_AUTO_COPY_EN, 0x69);
    ...
    for (list = 0; list < partition; list++) {
        ...
        for (i = 0; i < count; i++) {
            ...
            ret = nt36675_set_page(SpbContext, SRAM_addr);
            if (ret) {
                Trace(
                    TRACE_LEVEL_INFORMATION,
